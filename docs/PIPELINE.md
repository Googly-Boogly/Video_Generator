...
| 4b | AI refine (multi-agent) | `refine.py` | ✅ | Optional, user-triggered CrewAI crew critiques + rewrites the storyboard/narration. Long boards (>30 scenes) apply notes in length-preserving **chunks** (a chunk never drops scenes) so a 10-min film can't collapse; small boards use one Showrunner pass with a scene-count guard |
