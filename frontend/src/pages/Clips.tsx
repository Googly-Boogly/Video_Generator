...
export default function Clips() {
  ...
  const startPolling = useCallback(() => {
    ...
    pollRef.current = window.setInterval(async () => {
      await loadAll();
      // The video task commits atomically at the very end (minutes later), so scene
      // statuses don't reflect "generating" mid-run. Track the JOB instead: keep
      // polling while a video job is queued/running, stop once it settles.
      let active = false;
      if (id) {
        const jobs = await api.jobsForProject(id);
        active = jobs.some(
          (j) => j.type === "video" && (j.status === "queued" || j.status === "running")
        );
      }
      if (!active && pollRef.current) {
        ...
        setBusy(false);
        await loadAll(); // final refresh so the finished clips render
      }
    }, 2000);
  }, [id, loadAll]);
